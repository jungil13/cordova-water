...
<header id="main-header" class="fixed top-0 left-0 right-0 z-50 transition-all duration-500 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 [&.scrolled]:!bg-primary [&.scrolled]:border-primary/30">
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
            ...
            <button id="mobile-menu-btn" class="md:hidden hamburger flex flex-col gap-1.5 p-2 rounded-lg" aria-label="Menu">
                <span class="block w-6 h-0.5 bg-slate-700 rounded"></span>
                <span class="block w-6 h-0.5 bg-slate-700 rounded"></span>
                <span class="block w-6 h-0.5 bg-slate-700 rounded"></span>
            </button>
        </div>
    ...
    <div id="mobile-nav" class="hidden md:hidden border-t border-slate-200 bg-white/95">
        <ul class="px-4 py-4 space-y-1">
            <li><a href="<?= BASE_URL ?>/index.php" class="block px-4 py-3 rounded-lg font-medium">Home</a></li>
            <li><a href="<?= BASE_URL ?>/about.php" class="block px-4 py-3 rounded-lg font-medium">About</a></li>
            <li><a href="<?= BASE_URL ?>/services.php" class="block px-4 py-3 rounded-lg font-medium">Services</a></li>
            <li><a href="<?= BASE_URL ?>/contact.php" class="block px-4 py-3 rounded-lg font-medium">Contact</a></li>
            <li><a href="<?= BASE_URL ?>/payment.php" class="block px-4 py-3 rounded-lg font-medium">Payment</a></li>
            <?php if ($user): ?>
            <li><a href="<?= BASE_URL ?>/dashboard/" class="block px-4 py-3 rounded-lg font-medium">Dashboard</a></li>
            <?php if (isAdmin()): ?><li><a href="<?= BASE_URL ?>/admin/" class="block px-4 py-3 rounded-lg font-medium">Admin</a></li><?php endif; ?>
            ...
            <li><a href="<?= BASE_URL ?>/login.php" class="block px-4 py-3 rounded-lg font-medium">Login</a></li>
            <?php endif; ?>
        </ul>
    </div>
</header>
<style>
#main-header.scrolled .nav-link{color:rgba(255,255,255,0.9)}#main-header.scrolled .nav-link:hover{color:#fff;background:rgba(255,255,255,0.15)}#main-header.scrolled .nav-link.active,#main-header.scrolled .nav-link[class*="bg-primary"]{color:#fff;background:rgba(255,255,255,0.2)}#main-header.scrolled .nav-link-cta{background:#fff;color:#0A4D68}#main-header.scrolled .brand-name{color:#fff!important}.hamburger span{transition:all .3s}.hamburger.active span:nth-child(1){transform:rotate(45deg) translate(6px,6px)}.hamburger.active span:nth-child(2){opacity:0}.hamburger.active span:nth-child(3){transform:rotate(-45deg) translate(6px,-6px)}
</style>
